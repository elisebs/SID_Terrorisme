...
fichier = open("C:/Users/Sibylle/Documents/M1_SID/Semestre2/tdb/BDONNEES/insertion_code_article.txt", "a")
for i in range(len(real_data_final)):
    ...
    if id_article != real_data_final[i-1].get('id_art'):
        ...
        year=str(year_i)
            #date_pub = date(year, day, month)
        fichier.write("EXECUTE INSERTION_ARTICLE {0},'{1}','{2}-{3}-{4}';\n".format(id_article,name_np,year,month,day))
fichier.close()
...
for i in range(len(real_data_final)):
    ...
    if id_article != real_data_final[i-1].get('id_art'):
        ...
        if (len(month)==1):
            month="0"+month
        year=str(year_i)
        writer.writerow((id_article, "'"+name_np+"'", "'"+year+"-"+month+"-"+day+"'"))
file.close()
...
fichier = open("C:/Users/Sibylle/Documents/M1_SID/Semestre2/tdb/BDONNEES/insertion_code_word.txt", "a")
for i in range(len(real_data_final)):
    id_word=i+1
                                                ...
                                                if lemma.find(']') == -1 :
                                                    ...
                                                    if (len(lemma)<15): 
                                                        if (len(lemma)>1):
                                                            ...
                                                            fichier.write("EXECUTE INSERTION_WORD {0},'{1}','{2}','{3}',{4},{5};\n".format(id_word,word,lemma,pos_tag,position,id_article))
fichier.close()
...
fichier = open("C:/Users/Sibylle/Documents/M1_SID/Semestre2/tdb/BDONNEES/insertion_code_localisation.txt", "a")
...
fichier.write("EXECUTE INSERTION_LOCALISATION {0},'{1}','{2}',{3};\n".format(id_localisation,country,city,id_article))
fichier.close()
