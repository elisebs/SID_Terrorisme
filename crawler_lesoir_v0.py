     ...
     for url_article in list_url_articles:
        ...
        for a in soup.find_all('a'):             # find the authors
            if a.get('href') != None:
                if "dpi-authors"  in a.get('href').split('/'):
                    l = a.get('href').split('/')[-1]
                    authors.append(l.split('-')[0] + ' '+ l.split('-')[1])
        if len(authors)==0:
             authors.append('')
        ...
        for balise_time in soup.find_all('time'):     # find publication's date
            if 'pubdate' in balise_time.get('class'):
                ...
                date_publication.append(balise_time.get('datetime').split('T')[0])      
...
def recovery_new_articles_lpt(file_target="C:/Users/cmisid/Documents/TableauDeBord/LESOIR/" + str(date.datetime.now().date()) + "/"):
     
    list_url_articles = []
    j = 0
    for i in range(0,1650,10):
        j=j+1
        url1 = 'http://www.lesoir.be/archives/recherche?datefilter=lastyear&sort=date+desc&start='+str(i)+'&word=terrorisme'
        soup1 = utils.recovery_flux_url_rss(url1)
        
        for a in soup1.find_all('a'):
           l=a.get('href')
           if '/archive/' in l.split('d'):
                url = 'http://www.lesoir.be' + l
                list_url_articles.append(url)
            
             
    ######################
        url2 = 'http://www.lesoir.be/archives/recherche?datefilter=lastyear&sort=date+desc&start='+str(i)+'&word=attentat'
...
if __name__ == '__main__':
    file_target = "C:/Users/cmisid/Documents/TableauDeBord/LESOIR/" + str(date.datetime.now().date()) + "/"
