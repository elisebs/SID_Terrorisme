...
for article in file_article:
    cursor.execute("exec dbo.INSERTION_ARTICLE @pid_article_a = "
                   + article[0] + ", @pname_np = " + article[1]
                   + ", @pdate_pub = " + article[2])
    cursor.commit()
# fermer le fichier
f.close()
...
for loc in file_loc:
    cursor.execute("exec dbo.INSERTION_LOCALISATION @pid_localisation_l = "
                   + loc[0] + ", @pcountry = " + loc[1] + ", @pcity = " +
